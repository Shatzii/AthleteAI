...
// POST /api/computer-vision/realtime/start
// Start real-time video analysis
router.post('/realtime/start', authenticateToken, async (req, res) => {
    try {
        const { streamId, sport, position, athleteId } = req.body;

        if (!streamId || !sport || !position) {
            return res.status(400).json({
                success: false,
                error: 'streamId, sport, and position are required'
            });
        }

        // Start real-time analysis
        const session = await computerVisionService.startRealTimeAnalysis(
            streamId,
            sport,
            position,
            athleteId || req.user?.id,
            (analysisResult) => {
                // In a real implementation, this would send results via WebSocket
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to start real-time analysis',
            details: error.message
        });
    }
});

// POST /api/computer-vision/realtime/stop
// Stop real-time video analysis
router.post('/realtime/stop', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await computerVisionService.stopRealTimeAnalysis(streamId);
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to stop real-time analysis',
            details: error.message
        });
    }
});

// GET /api/computer-vision/realtime/stats
// Get real-time analysis statistics
router.get('/realtime/stats', authenticateToken, async (req, res) => {
    try {
        const stats = computerVisionService.getRealTimeStats();
...
// POST /api/computer-vision/realtime/frame
// Process a single frame for real-time analysis (for WebRTC integration)
router.post('/realtime/frame', authenticateToken, async (req, res) => {
    try {
        ...
        if (!streamId || !frameData) {
            return res.status(400).json({
                success: false,
                error: 'streamId and frameData are required'
            });
        }

        // Process single frame
        const analysis = await computerVisionService.processSingleFrame(streamId, frameData, timestamp);
        ...
        console.error('Error processing real-time frame:', error);
        res.status(500).json({
            success: false,
